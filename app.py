...
def clean_data(df, column_to_check):
    ...
    return df.drop(columns=['count_Row'], errors='ignore')
...
def login():
    if request.method == 'POST':
        ...
        # Convert both input username and stored username to lowercase for case-insensitive comparison
...
# Route to preview duplicate count and show the full table
...
def preview_duplicates():
    ...
    # Mark duplicates in the specified column
...
def process_file():
    ...
    if action == 'remove_duplicates':
        ...
        # Remove 'Duplicate_Value' column before saving
        cleaned_df = cleaned_df.drop(columns=['Duplicate_Value'], errors='ignore')
